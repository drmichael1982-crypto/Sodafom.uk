...
import { getRememberedChildName } from '@/lib/archie-local';
...
export default function ArchieHelper({ gameMode = false }: { gameMode?: boolean }) {
  ...
  async function sendMessage(text: string) {
    ...
    // React state updates are asynchronous: this lock also covers rapid mic/click events.
    ...
    try {
      ...
      if (gameTitle) {
        ...
        if (currentQuestion) contextPrompt += ` The current question is: "${currentQuestion}".`;
      ...
      const localHint = gameTitle && hintRequest ? (currentQuestion
        ...
        : `You are playing ${gameTitle}. Read the instructions carefully, take your time, and try one step at a time. I'm right here if you need me.`) : undefined;
      const history = [...messages, userMessage]
        .filter((m) => m.content.trim())
        .map((m) => ({ role: m.role, content: m.content }));
      const reply = await askArchie({ messages: history, systemExtra: contextPrompt, localHint, signal: controller.signal });
      if (controller.signal.aborted) return;
      setMessages((prev) => prev.map((m) => m.id === assistantId ? { ...m, content: reply.text } : m));
      const parsed = parseArchieMessage(reply.text);
      if (parsed.text) speak(parsed.text);
    } catch (err) {
      if (controller.signal.aborted) return;

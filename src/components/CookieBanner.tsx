...
import { isCapacitor } from '@/lib/config';
...
function initC2Tracking(): void {
  if (typeof window === 'undefined' || window.__SCC_INIT__ || isCapacitor) return;
...
export default function CookieBanner() {
  ...
  React.useEffect(function checkConsent() {
  ...
  React.useEffect(function exposeRevokeFunction() {
    if (typeof window === 'undefined') return;
    window.revokeAnalyticsConsent = revokeConsent;
    return () => { delete window.revokeAnalyticsConsent; };
  }, []);

  React.useEffect(function listenForBuilderBuildComplete() {
      ...
      <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            <h3 className="text-sm font-bold text-gray-900 mb-1">Cookie Consent</h3>

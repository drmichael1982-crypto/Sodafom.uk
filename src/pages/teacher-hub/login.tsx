...
export default function TeacherHubLoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (registering) {
        ...
        setRegistering(false); setMessage('Your teacher account is ready. Sign in to open your class.'); setPassword(''); return;
      }
  ...
  return <main className="min-h-screen bg-gradient-to-br from-primary/10 to-accent/10 px-4 py-12">
    ...
    <div className="mx-auto max-w-lg rounded-3xl border border-border bg-card p-6 shadow-xl sm:p-8">
      ...
      <p className="my-4">This private teacher workspace stores teacher-entered comments and reviewed marks. It does not upload work photos or call AI.</p>
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        {registering && <><p className="text-sm">Use at least 12 characters. An active school access key is required to create a teacher account.</p><label className="block">School access key<input className={input} required maxLength={64} value={licenceKey} onChange={e => setLicenceKey(e.target.value)} /></label></>}
        <button disabled={busy} className="w-full rounded-xl bg-primary p-3 font-bold text-primary-foreground disabled:opacity-50">{busy ? 'Please wait…' : registering ? 'Create teacher account' : 'Sign in'}</button>
      </form>

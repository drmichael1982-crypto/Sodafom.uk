    ...
    <div className="mx-auto max-w-4xl">
      ...
      <motion.article initial={{ rotateY: -80, scale: .72, opacity: 0 }} animate={{ rotateY: 0, scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 115, damping: 18 }} className="overflow-hidden rounded-[2rem] border-8 border-amber-200 bg-[#fff9e8] shadow-2xl">
        <div className="grid min-h-[620px] md:grid-cols-2">
          <div className="relative min-h-72 overflow-hidden bg-indigo-950"><motion.img key={`${story.title}-${page}-${archiesView}`} src="/assets/approved/stories.png" alt="Archie’s illustrated storybook shelf" className="absolute inset-0 h-full w-full object-cover object-center" initial={{ scale: archiesView ? 1.42 : 1.16, x: page % 2 ? '5%' : '-5%', opacity: .55 }} animate={{ scale: archiesView ? 1.26 : 1.04, x: archiesView ? (page % 2 ? '-13%' : '13%') : (page % 2 ? '-3%' : '3%'), opacity: 1 }} transition={{ duration: 7, ease: 'linear' }} /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/45 to-transparent" />{[0, 1, 2, 3, 4].map((star) => <motion.span key={star} className="absolute h-3 w-3 rounded-full bg-yellow-200 shadow-[0_0_16px_5px_rgba(253,224,71,.9)]" initial={{ left: `${12 + star * 18}%`, top: '88%', opacity: 0 }} animate={{ top: '10%', opacity: [0, 1, 1, 0], scale: [0.4, 1.2, .8] }} transition={{ duration: 3.5 + star * .4, delay: star * .3, repeat: Infinity }} />)}<motion.div className="absolute bottom-5 left-5 h-16 w-16 rounded-full border-4 border-yellow-200 bg-amber-400 shadow-[0_0_32px_9px_rgba(250,204,21,.8)]" animate={{ rotate: 360, scale: [1, 1.16, 1] }} transition={{ rotate: { duration: 9, repeat: Infinity, ease: 'linear' }, scale: { duration: 1.8, repeat: Infinity } }} />{!archiesView && <><motion.div className="absolute bottom-2 left-2" animate={turning ? { x: ['0%', '290%'], rotate: [0, -8, 8, 0] } : { y: [0, -7, 0], rotate: [0, -2, 2, 0] }} transition={turning ? { duration: .7, ease: 'easeInOut' } : { duration: 2.2, repeat: Infinity }}><ArchieCharacter size={96} speaking={turning} /></motion.div><motion.img src="/assets/cartoon/friends/daisy.png" alt="Daisy the dog" className="absolute bottom-3 right-4 h-20 w-20 object-contain drop-shadow-xl" animate={turning ? { x: ['0%', '-300%'], rotate: [0, 10, -10, 0] } : { y: [0, -8, 0], rotate: [-3, 3, -3] }} transition={turning ? { duration: .65, ease: 'easeInOut' } : { duration: 1.8, repeat: Infinity }} /></>}<div className="absolute bottom-4 left-4 rounded-full bg-white/90 px-4 py-2 font-black">{archiesView ? '👀 Looking through Archie’s eyes' : '✨ Moving illustrated story page'}</div></div>

...
export default function Products() {
  ...
  if (products.length === 0) {
    return <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-xl font-bold text-gray-900 mb-2">{t('stripe.no_products_title')}</h2>
  ...
  return <>
      <Helmet>
        <title>Sodafom Subscriptions — Plans for Families & Schools</title>
        <meta name="description" content="Choose a Sodafom plan. Monthly and annual subscriptions for families, plus school licences. Fun maths, spelling and reading games for ages 5–13." />
        <link rel="canonical" href="https://sodafom.uk/Products" />
        <meta property="og:title" content="Sodafom Subscriptions — Plans for Families & Schools" />
        <meta property="og:description" content="Choose a Sodafom plan. Monthly and annual subscriptions for families, plus school licences." />
        <meta property="og:image" content="https://sodafom.uk/og-image.png" />
        <meta property="og:url" content="https://sodafom.uk/Products" />
        ...
        <script type="application/ld+json">{JSON.stringify({
          ...
          '@type': 'WebPage',
          '@id': 'https://sodafom.uk/Products#webpage',
          name: 'Sodafom Subscriptions',
          url: 'https://sodafom.uk/Products',
          description: 'Choose a Sodafom plan. Monthly and annual subscriptions for families, plus school licences.',
          isPartOf: {
            '@id': 'https://sodafom.uk/#website'
          },
          about: {
            '@id': 'https://sodafom.uk/#organization'
          }
        })}</script>
      </Helmet>
            ...
            {products.map(product => <div key={product.id} className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-2">
                    <button onClick={() => openProductDialog(product)} className="w-full py-2 px-4 rounded-lg font-bold border-2 border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">
                      {t('stripe.btn_view_details')}
                    </button>
                    {ENABLE_CART ?
                // Show "Add to Cart" + "Buy Now"
                <div className="flex gap-2">
                        <button onClick={() => handleAddToCart(product)} className="flex-1 py-3 px-4 rounded-lg font-bold border-2 border-blue-600 text-blue-600 hover:bg-blue-50 transition-colors">
                          {t('stripe.btn_add_to_cart')}
                        </button>
                        <button onClick={() => handleCheckout(product)} disabled={checkingOut === product.priceId} className={`flex-1 py-3 px-4 rounded-lg font-bold transition-colors ${checkingOut === product.priceId ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                ...
                <button onClick={() => handleCheckout(product)} disabled={checkingOut === product.priceId} className={`w-full py-3 px-4 rounded-lg font-bold transition-colors ${checkingOut === product.priceId ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
                        {checkingOut === product.priceId ? <span className="flex items-center justify-center">
                            <svg className="animate-spin -ml-1 mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24">
                              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
      ...
      {isDialogOpen && selectedProduct && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" onClick={closeDialog}>
          <div className="bg-white rounded-2xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6">
              ...
              {selectedProduct.description && <div className="mb-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-2">{t('stripe.modal_description_title')}</h3>
          ...
          }} disabled={checkingOut === selectedProduct.priceId} className={`w-full py-4 px-6 rounded-lg font-bold text-lg transition-colors ${checkingOut === selectedProduct.priceId ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>

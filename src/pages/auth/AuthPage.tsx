...
export default function AuthPage({
  ...
  async function handleEmailSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (result.error) {
        ...
        const friendly =
          /user already exists|email.*already|duplicate/i.test(raw)
            ? 'An account with that email already exists. Try signing in instead.'
            : /invalid.*email|email.*invalid/i.test(raw)
            ? 'Please enter a valid email address.'
            : /invalid.*password|password.*incorrect|credentials/i.test(raw)
            ? 'Incorrect email or password. Please try again.'
            : /password.*weak|password.*short|password.*length/i.test(raw)
            ? 'Password must be at least 8 characters and include uppercase, a number, and a special character.'
            : /too many.*request|rate.?limit/i.test(raw)
  ...
  async function handleSocialSignIn(provider: string) {
    ...
    if (!isInIframe()) {
      ...
      try {
        ...
        setError(`Failed to sign in with ${(Object.hasOwn(PROVIDER_CONFIG, provider) ? PROVIDER_CONFIG[provider as keyof typeof PROVIDER_CONFIG] : undefined)?.name || provider}`);
    ...
    const providerName: string = (Object.hasOwn(PROVIDER_CONFIG, provider) ? PROVIDER_CONFIG[provider as keyof typeof PROVIDER_CONFIG] : undefined)?.name || provider;
  ...
  return <>
      <Helmet>
        <title>{mode === 'login' ? 'Sign In' : 'Create Account'} — Sodafom</title>
        <meta name="description" content="Sign in or create your Sodafom account to access fun learning games for children aged 5–13." />
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted py-12 px-4">
      <h1 className="sr-only">{mode === 'login' ? 'Sign In' : 'Create Account'} — Sodafom</h1>
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl shadow-lg border border-border p-8">
          <h2 className="text-2xl font-black text-foreground text-center mb-1" style={{ fontFamily: 'var(--font-heading)' }}>
            {showEmail ? isLogin ? 'Welcome back!' : 'Create your account' : 'Welcome'}
          </h2>
          {!showEmail && <p className="text-muted-foreground text-center text-sm mb-6">Sign in to continue learning</p>}
          {showEmail && <p className="text-muted-foreground text-center text-sm mb-6">{isLogin ? 'Sign in to continue' : 'Start your free 7-day trial'}</p>}

        {error && <div className="mb-4 p-3 bg-destructive/10 border border-destructive/30 text-destructive rounded-xl text-sm font-medium">
            {error}
          </div>}

        {oauthNotice && <div className="mb-4 p-3 bg-primary/10 border border-primary/30 text-primary rounded-xl text-sm">
            {oauthNotice}
          </div>}
        ...
        {showOAuth && <div className="space-y-3 mb-6">
            ...
            const config = Object.hasOwn(PROVIDER_CONFIG, provider) ? PROVIDER_CONFIG[provider as keyof typeof PROVIDER_CONFIG] : undefined;
            ...
            return <button key={provider} onClick={() => handleSocialSignIn(provider)} disabled={loading || socialLoading !== null} className={`w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${bgClass} ${hoverClass}`}>
        ...
        {showEmail && <>
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              {!isLogin && <div>
                  <label htmlFor="name" className="block text-sm font-bold text-foreground mb-1">Your name</label>
                  <input id="name" type="text" value={name} onChange={e => setName(e.target.value)} disabled={loading} className="w-full px-3 py-2.5 border border-border rounded-xl bg-background focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:bg-muted text-sm" placeholder="Your name" />
                </div>}
              <div>
                <label htmlFor="email" className="block text-sm font-bold text-foreground mb-1">Email address</label>
                <input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} required disabled={loading} className="w-full px-3 py-2.5 border border-border rounded-xl bg-background focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:bg-muted text-sm" placeholder="you@example.com" />
              </div>
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label htmlFor="password" className="block text-sm font-bold text-foreground">Password</label>
                  {isLogin && (
                    <Link to="/hub/forgot-password" className="text-xs text-primary font-bold hover:underline">
                      Forgot password?
                    </Link>
                  )}
                </div>
                <input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={isLogin ? undefined : 8} disabled={loading} className="w-full px-3 py-2.5 border border-border rounded-xl bg-background focus:outline-none focus:ring-2 focus:ring-primary/40 disabled:bg-muted text-sm" placeholder={isLogin ? '••••••••' : 'Min 8 chars, A-Z, 0-9, special'} />
                {!isLogin && <p className="mt-1 text-xs text-muted-foreground">Must include uppercase, number, and special character</p>}
              </div>
              <button type="submit" disabled={loading || socialLoading !== null} className="w-full py-3 px-4 bg-primary text-primary-foreground font-black rounded-xl hover:opacity-90 disabled:opacity-50 transition-opacity text-sm">
            ...
            <p className="mt-5 text-center text-sm text-muted-foreground">
              ...
              <a href={isLogin ? '/signup' : '/login'} className="text-primary hover:underline font-bold">
                {isLogin ? 'Sign up free' : 'Sign in'}
              </a>
            </p>
          </>}

        {!showEmail && <p className="mt-6 text-center text-xs text-muted-foreground">

...
export default function CartPage() {
  ...
  return <>
      <Helmet>
        <title>Your Cart — Sodafom</title>
        <meta name="description" content="Review your Sodafom subscription items and proceed to checkout." />
        <link rel="canonical" href="https://sodafom.uk/cart" />
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">{t('stripe.empty_cart_title')}</h2>
            <p className="text-gray-600 mb-6">{t('stripe.empty_cart_message')}</p>
            <Link to="/" className="inline-block px-6 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors">
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-6">
              {cart.map((item, index) => <div key={item.id} className={`flex items-center gap-4 p-6 ${index > 0 ? 'border-t border-gray-100' : ''}`}>
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-300 hover:bg-gray-50 text-lg font-bold text-gray-900">
                      −
                    </button>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-4">{t('stripe.order_summary_title')}</h2>
              ...
              <button onClick={handleCheckout} disabled={checkingOut} className={`w-full py-4 px-6 rounded-lg font-bold text-lg transition-colors ${checkingOut ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>

  ...
  const Museum = () => {
    ...
    return <main className="min-h-screen bg-gradient-to-b from-sky-200 via-cyan-50 to-amber-100 text-slate-900">{top('Museum Explorer','Walk through a virtual museum with Archie')}
      {!chosen ? <div className="mx-auto max-w-5xl px-4 pb-8"><div className={`${panel} overflow-hidden bg-gradient-to-b from-cyan-50 to-white p-5`}>
        ...
        <div className="mt-6 rounded-[2.5rem] border-8 border-amber-700 bg-gradient-to-b from-amber-100 via-amber-50 to-sky-100 p-5 shadow-2xl"><div className="mb-4 text-center text-xl font-black text-amber-950">🏛️ Archie’s Museum Building — choose a colourful gallery door</div><div className="grid grid-cols-2 gap-4 sm:grid-cols-3">{(Object.entries(MUSEUMS) as [MuseumKey, typeof MUSEUMS[MuseumKey]][]).map(([key,museum])=><button key={key} onClick={()=>openMuseum(key)} style={{background:`linear-gradient(145deg, ${museum.colour}, #0f172a)`}} className="aspect-square rounded-t-[2.5rem] rounded-b-2xl border-8 border-white p-4 text-white shadow-xl transition hover:-translate-y-1 active:scale-95"><div className="text-5xl">{museum.icon}</div><div className="mt-2 font-black">{museum.title}</div><div className="mt-1 text-xs font-bold">Open gallery</div></button>)}</div></div>
        <button onClick={()=>navigate('/games/colour-book')} className="mt-6 w-full rounded-[2rem] border-4 border-amber-700 bg-gradient-to-r from-amber-300 via-pink-300 to-cyan-300 p-5 text-center text-lg font-black text-blue-950 shadow-xl transition hover:-translate-y-1 active:scale-95">🎨 Museum Colouring Books — dinosaurs, ancient Egypt, space and nature pictures</button>

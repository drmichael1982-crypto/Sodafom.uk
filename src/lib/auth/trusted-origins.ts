...
// Exact origins used by the packaged Capacitor app. No arbitrary ports.
...
/**
 * Returns only exact origins which are safe to use with credentialed requests.
 * Preview and custom HTTPS frontends must be opt-in through SODAFOM_TRUSTED_ORIGINS.
 */
export function trustedOrigins(requestOrigin?: unknown, env: Environment = process.env): string[] {
  ...
  // Development supports local Vite ports without extending production trust.

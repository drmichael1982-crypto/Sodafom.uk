...
export function findApplicableFormatOverride(
  ...
  const sidecar = (Object.hasOwn(bundle.scopes, scope.key) ? bundle.scopes[scope.key as keyof typeof bundle.scopes] : undefined) ?? EMPTY_FORMAT_OVERRIDE_SIDECAR
  const entry = Object.hasOwn(sidecar.overrides, devId) ? sidecar.overrides[devId as keyof typeof sidecar.overrides] : undefined
...
export function buildFormatOverrideStyle(marks: FormatOverrideMarks): CSSProperties {
  const lineHeight: string | undefined = marks.fontSize
    ? (Object.hasOwn(FONT_SIZE_LINE_HEIGHT, marks.fontSize) ? FONT_SIZE_LINE_HEIGHT[marks.fontSize] : undefined)
    : undefined
  return {

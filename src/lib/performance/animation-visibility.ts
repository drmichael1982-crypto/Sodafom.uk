...
function createPool(doc: Document): VisibilityPool {
  ...
  const observer = Observer ? new Observer((entries) => {
    for (const entry of entries) {
      ...
      if (!target) continue; // Ignore notifications queued before cleanup.
  ...
  return {
    subscribe(element, listener) {
      ...
      // Each call owns its subscription, even for the same listener/element.
...
export function observeAnimationVisibility(
  ...
  // Safe to import during SSR; all browser setup is deferred until subscription.
  if (!doc?.defaultView) {
    listener(false);
    return () => {};
  }

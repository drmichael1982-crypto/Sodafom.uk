...
export function requestMetadata(req: Request): { method: string; area: string } {
  ...
  const area = segments[1] === 'api'
    ? (AREAS.has(segments[2]) ? segments[2] : 'api') : 'page';
...
export function httpSecurity(req: Request, res: Response, next: NextFunction) {
  ...
  // Restricts other origins; it never grants camera/microphone permission.
  ...
  } else if (api && !SAFE_METHODS.has(req.method) &&
             ['cross-site', 'same-site'].includes(String(req.headers['sec-fetch-site'] || ''))) {
    // Headerless native/server requests (including signed webhooks) still work.
    // Browser-originated writes must not bypass checks by dropping Origin.
...
export function apiErrorHandler(error: unknown, req: Request, res: Response, next: NextFunction) {
  ...
  const message = status === 413 ? 'Request too large' : status === 400 ? 'Invalid request body' : 'Internal server error';

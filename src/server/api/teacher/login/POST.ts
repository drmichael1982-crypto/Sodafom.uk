...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    if (body.action === 'logout') {
      ...
      if (token) await db.delete(teacherSessions).where(or(eq(teacherSessions.token, tokenHash(token)), eq(teacherSessions.token, token)));
      return res.json({ success: true });
    }
    if (!allowAttempt(req, res, 'teacher-login', 30)) return;
    const email = normaliseEmail(body.email), password = passwordInput(body.password);
    const [teacher] = await db.select().from(teacherAccounts).where(eq(teacherAccounts.email, email)).limit(1);
    if (!teacher) { await hashTeacherPassword(password); return res.status(401).json({ error: 'Invalid email or password.' }); }
    ...
    if (verification.needsUpgrade) await db.update(teacherAccounts).set({ passwordHash: await hashTeacherPassword(password) }).where(eq(teacherAccounts.id, teacher.id));
    const token = randomBytes(48).toString('hex');
    await db.insert(teacherSessions).values({ teacherId: teacher.id, token: tokenHash(token), expiresAt: new Date(Date.now() + 8 * 60 * 60 * 1000) });
    return res.json({ token, teacher: { id: teacher.id, name: teacher.name, email: teacher.email, className: teacher.className ?? 'My Class' } });
  } catch (error) { return schoolError(res, error, 'Sign-in is unavailable. Please try again.'); }
}

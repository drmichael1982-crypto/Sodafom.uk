...
import { decodeReview, REVIEW_PREFIX, summariseProgress, SCHOOL_POLICY, type SavedReview } from '@/lib/teacher-school';
...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    const comments = notes.filter(note => !note.noteText.startsWith(REVIEW_PREFIX));
    const progress = summariseProgress(activity, reviews);
    const recommendations = getGameRecommendations(comments.map(note => note.noteText).join(' '), activity.map(entry => entry.gameId));
    return res.json({ student, activity, notes: comments, reviews, progress,
      ...
      historyScope: 'Latest 500 activity records and latest 500 comments/work reviews. Not a lifetime report.', schoolPolicy: SCHOOL_POLICY });

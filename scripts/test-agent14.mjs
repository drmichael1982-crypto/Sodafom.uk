...
test('registration requires and retains a valid school access key', async () => { const f = fixture(); f.queue.push([{ id: 19, expiresAt: null }], [], [{ id: 7 }]); const res = f.res(); await f.load('src/server/api/teacher/register/POST.ts').default(f.req({ name: 'Test Teacher', email: 'teacher@example.test', className: 'Class A', password: 'long-test-password', licenceKey: 'SODA-TEST' }), res); assert.equal(res.statusCode, 201); assert.equal(f.calls.at(-1).values[0].licenceId, 19); assert.equal(f.calls.length, 3); });
test('registration rejects a missing or invalid school key without bypass', async () => { const missing = fixture(); const missingRes = missing.res(); await missing.load('src/server/api/teacher/register/POST.ts').default(missing.req({ name: 'Test Teacher', email: 'teacher@example.test', className: 'Class A', password: 'long-test-password' }), missingRes); assert.equal(missingRes.statusCode, 400); assert.equal(missing.calls.length, 0); const invalid = fixture(); invalid.queue.push([]); const invalidRes = invalid.res(); await invalid.load('src/server/api/teacher/register/POST.ts').default(invalid.req({ name: 'Test Teacher', email: 'teacher@example.test', className: 'Class A', password: 'long-test-password', licenceKey: 'invalid' }), invalidRes); assert.equal(invalidRes.statusCode, 400); assert.equal(invalid.calls.length, 1); });

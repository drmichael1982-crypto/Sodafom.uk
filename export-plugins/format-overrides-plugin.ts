  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.watcher.on('change', (filePath: string) => {
        const relative = path.relative(sidecarRoot, filePath);
        if (relative.startsWith('..') || !filePath.endsWith('.json')) return;
        const moduleNode = server.moduleGraph.getModuleById(RESOLVED_FORMAT_OVERRIDES_MODULE_ID);
        if (moduleNode) server.moduleGraph.invalidateModule(moduleNode);
        server.ws.send('format-overrides:update', readSidecarBundle(root));
      });
    },
  };
}
